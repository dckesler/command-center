...
- **`-s <session>`** — open the window in another tmux session (used by the command center TUI and by `project-task`, which puts ticket worktrees inside a project's session).
